...
<div class="modul recruitment recr-add">
	...
	<div class="title">
		<h3>Заявка от <?php echo $recruitment->account->nickname ?>, <?php echo date('d.m.Y', $recruitment->time+Yii::app()->params["moscow"]) ?></h3>
	</div>
	<div class="cell">

	<ul class="recruitment-nav">
		<li><a class="recr-index" href="/recruitment">Военкомат</a></li>
		<li class="active"><?php echo CHtml::link('Подать заявку', array('recruitment/add'), array('class'=>'recr-add')) ?></li>
		<li><?php echo CHtml::link('Требования к кандидатам', array('page/index', 'id'=>'requirements'), array('class'=>'recr-cand')) ?></li>
	<? if (Yii::app()->user->checkAccess('0') || Yii::app()->user->checkAccess('1') || Yii::app()->user->checkAccess('4') || Yii::app()->user->checkAccess('8')){ ?>
	<li><?php echo CHtml::link('Статистика любого игрока', array('recruitment/stat'), array('class'=>'recr-stat')) ?></li>
	<? } ?>
	</ul>

		<div class="table-data">
			<p>Статистика актуальна на момент последней авторизации пользователя</p>
			<? $this->widget('zii.widgets.CDetailView', array(
					...
					'attributes'=>array(
							...
							array(
								'label'=>'Регион',
								'type'=>'raw',
								'value'=>$recruitment->account->region,
							),
							'experience',
							'about',
							array(
